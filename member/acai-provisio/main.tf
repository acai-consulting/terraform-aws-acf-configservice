...
locals {
  ...
  delivery_target_s3 = var.aws_config_settings.aws_config.delivery_channel_target.central_s3 != null
  provisio_package_files = merge(
    {
      "aws_config.tf" = templatefile("${path.module}/templates/aws_config.tf.tftpl", {
        ...
        config_s3_delivery_bucket_kms_cmk_arn = local.delivery_target_s3 ? var.aws_config_settings.delivery_channel_target.central_s3.bucket_kms_cmk_arn : ""
